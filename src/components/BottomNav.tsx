...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-nav shadow-nav border-t border-border/50 z-50 md:top-0 md:bottom-auto md:border-t-0 md:border-b">
      <div className="max-w-7xl mx-auto flex items-center justify-around md:justify-center md:gap-1 py-2 px-1">
        {navItems.map(({ path, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`nav-item flex-1 md:flex-none md:flex-row md:px-4 ${isActive ? 'nav-item-active' : 'text-nav-foreground hover:text-foreground'}`}
            >
